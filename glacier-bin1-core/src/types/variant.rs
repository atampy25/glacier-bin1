...
use crate::{
	...
	types::{
		...
		resource::{TResourcePtr, ZResourceID, ZRuntimeResourceID}
	}
};
...
const fn make_array_ty(contained_ty: &str) -> ([u8; CONST_STR_BUF_SIZE], usize) {
	...
	if START.len() + contained_ty.len() + END.len() > CONST_STR_BUF_SIZE {
		panic!("type ID is too long to fit in buffer, glacier-bin1 needs to be updated");
	}
...
impl_variant!(ZResourceID, "ZResourceID");
