...
impl PropertyID {
	pub fn as_name(&self) -> Option<EcoString> {
		if let Some(known) = PROPERTIES.get(&self.0).copied() {
			Some(known.into())
		} else {
	...
	pub fn as_known(&self) -> Option<&'static str> {
		PROPERTIES.get(&self.0).copied()
	}

	pub fn is_known(name: &str) -> bool {
		PROPERTIES.contains_key(&crc32fast::hash(name.as_bytes()))
	}
}
...
impl From<EcoString> for PropertyID {
	fn from(value: EcoString) -> Self {
		...
		if PROPERTIES.contains_key(&hash) {
			Self(hash)
		} else {
