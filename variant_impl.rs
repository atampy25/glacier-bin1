...
trait DeserializeVariant: Send + Sync {
	fn type_id(&self) -> &str;
	fn deserialize_serde(&self, value: serde_json::Value) -> Result<Arc<dyn Variant>, String>;
	fn deserialize_bin1(&self, de: &mut Bin1Deserializer) -> Result<Arc<dyn Variant>, DeserializeError>;
}

struct VariantDeserializer<T: StaticVariant + Variant + DeserializeOwned + 'static + Send + Sync>(
...
impl<
	T: StaticVariant
		...
		+ Clone
		+ Serialize
> DeserializeVariant for VariantDeserializer<T>
{
	fn type_id(&self) -> &str {
		T::TYPE_ID
	}
	...
	fn deserialize_serde(&self, value: serde_json::Value) -> Result<Arc<dyn Variant>, String> {
		serde_json::from_value::<T>(value)
			...
			.map_err(|e| format!("invalid {}: {}", T::TYPE_ID, e))
	}
	...
	fn read(de: &mut Bin1Deserializer) -> Result<Self, DeserializeError> {
		...
		if type_id == "void" {
			...
			let deserializer = DESERIALIZERS
				.get(type_id)
				.ok_or_else(|| DeserializeError::UnknownType(type_id.into()))?;

			de.read_variant_ptr(|de| deserializer.deserialize_bin1(de))?.into()
		}
	}
}
				...
				V: serde::de::SeqAccess<'de>
			{
				...
				if let Some(deserializer) = DESERIALIZERS.get(&*type_id) {
					...
					let variant = deserializer
						.deserialize_serde(value.value.to_owned().into())
...
submit!(SEntityTemplateProperty, "SEntityTemplateProperty");
