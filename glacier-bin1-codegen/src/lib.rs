						...
						.map(|v| RustVariant {
							rust_name: {
								...
								if let Some(common_prefix) = &common_prefix {
									name = v.name.trim_start_matches(common_prefix).trim_start_matches('_');
								}

								if name.starts_with(|c: char| c.is_ascii_digit()) || name.to_lowercase() == "self" {
									format!(
										"_{}",
...
pub fn generate(scope: &mut Scope, types_json: &str, custom_types_json: &str, to_generate: &[&[&str]]) {
	...
	// Special cased
	for ty in ["ZResourceID", "ZRuntimeResourceID", "SEntityTemplateProperty"] {
	...
	let known_variants: Vec<String> = serde_json::from_str(include_str!("known_variants.json")).unwrap();

	for ty in to_generate.concat() {
		if let Some(pos) = structs.iter().position(|x| x.type_id == ty) {
			struct_queue.push_back((structs.remove(pos), false));
		}
	...
	{
		let is_variant = is_variant || known_variants.contains(&type_id);
			...
			for ty in tys {
				if ty == "ZVariant" || ty == "SEntityTemplateProperty" {
					for ty in &known_variants {
						if let Some(pos) = structs.iter().position(|x| x.type_id == *ty) {
							struct_queue.push_back((structs.remove(pos), true));
						}
					}
					...
					scope.import("glacier_bin1_core::types::resource", "TResourcePtr");
				} else if ty == "TypeID" {
